...
const Dashboard = () => {
  ...
  // Redirect to OTP if not authenticated
  useEffect(() => {
    if (!authLoading && !currentUser) {
      const returnUrl = typeof window !== 'undefined' ? window.location.pathname + window.location.search : '/ru/dashboard';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900 transition-colors">
        <div className="text-center">
          ...
          <p className="text-gray-900 dark:text-white">{t('dashboard.loadingDashboard', 'Загрузка...')}</p>
          <p className="text-gray-600 dark:text-gray-400 text-sm mt-2">{t('dashboard.checkingAuth', 'Проверка авторизации...')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!authLoading && !currentUser) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900 transition-colors">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 dark:border-blue-400 mx-auto mb-4"></div>
          <p className="text-gray-900 dark:text-white">{t('dashboard.redirectingToLogin', 'Перенаправление...')}</p>
        </div>
      </div>
    );
  }
