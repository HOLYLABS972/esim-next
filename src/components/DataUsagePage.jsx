    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-600">{t('dashboard.dataUsage.loading', 'Загрузка данных...')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 p-4">
        <div className="max-w-2xl mx-auto pt-8">
          ...
          >
            ...
            {t('dashboard.dataUsage.back', 'Назад')}
          </button>
          
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            <AlertCircle className="w-16 h-16 text-red-500 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('dashboard.dataUsage.errorLoadingData', 'Ошибка загрузки')}</h2>
            <p className="text-gray-600 mb-6">{error}</p>
            <button
              onClick={() => fetchDataUsage()}
              className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
            >
              {t('dashboard.dataUsage.tryAgain', 'Попробовать снова')}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto pt-8 pb-12">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6 text-white">
            <h1 className="text-2xl font-bold mb-2">{t('dashboard.dataUsage.title', 'Использование данных')}</h1>
          ...
          <div className="p-6 space-y-6">
            ...
            {dataInfo?.packageName && (
              <div className="text-center pb-4 border-b">
                ...
                <p className="text-sm text-gray-600 mt-1">
                  {t('dashboard.dataUsage.operator', 'Оператор')}: Roamjet
                </p>
              </div>
            )}
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center">
                ...
                <span className="text-gray-700 font-medium">{t('dashboard.dataUsage.status', 'Статус')}</span>
              </div>
              <span className={`px-4 py-1 rounded-full text-sm font-medium border ${getStatusColor(dataInfo?.status)}`}>
                {dataInfo?.status ? t(`dashboard.status.${dataInfo.status.toLowerCase()}`, dataInfo.status) : t('dashboard.unknown', 'Неизвестно')}
              </span>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <span className="text-gray-700 font-medium">{t('dashboard.dataUsage.dataUsage', 'Использование данных')}</span>
                </div>
              ...
              <div className="flex justify-between text-sm text-gray-600">
                <span>{t('dashboard.dataUsage.used', 'Использовано')}: {dataInfo?.dataUsed || 'N/A'}</span>
                <span>{t('dashboard.dataUsage.total', 'Всего')}: {dataInfo?.dataTotal || 'N/A'}</span>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg border border-green-200">
                <div className="flex items-center">
                  ...
                  <span className="text-gray-700 font-medium">
                    {dataInfo?.isUnlimited ? t('dashboard.dataUsage.unlimitedData', 'Безлимитные данные') : t('dashboard.dataUsage.dataAvailable', 'Данные доступны')}
                  </span>
                </div>
                ...
                {dataInfo.daysUsed > 0 && (
                  <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-sm text-gray-600">{t('dashboard.dataUsage.daysUsed', 'Дней использовано')}</span>
                    </div>
                ...
                {dataInfo.daysRemaining > 0 && (
                  <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-sm text-gray-600">{dataInfo.daysUsed > 0 ? t('dashboard.dataUsage.daysRemaining', 'Дней осталось') : t('dashboard.dataUsage.validityDays', 'Срок действия (дней)')}</span>
                    </div>
              ...
              <div className="p-4 bg-orange-50 rounded-lg border border-orange-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-700 font-medium">{t('dashboard.dataUsage.expiresOn', 'Истекает')}</span>
                  </div>
            ...
            {dataInfo?.lastUpdated && (
              <p className="text-xs text-gray-500 text-center">
                {t('dashboard.dataUsage.lastUpdated', 'Обновлено')}: {new Date(dataInfo.lastUpdated).toLocaleString(locale === 'ru' ? 'ru-RU' : 'en-US')}
              </p>
            )}
          </div>
        </div>
