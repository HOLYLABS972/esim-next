...
export default function ConfigPlansPage() {
  ...
  const handleShare = async (plan) => {
      ...
      try {
        const res = await fetch('/api/config/affiliates', {
          ...
          body: JSON.stringify({
            ...
            package_slug: String(slug),
            label: `${String(slug).slice(0, 20)}… (${pct}%)`,
          }),
        });
    ...
    try {
      ...
      toast.error('Could not copy – copy manually: ' + shareUrl);
    }
  };

  const syncFromAiralo = async () => {
    setSyncing(true);
    try {
      if (mode === 'topups') {
        // First sync all packages, then mark topups
        ...
        const res = await fetch('/api/config/sync-packages?wipe=true', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        });
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data?.error || 'Sync failed');
        toast.success(`Synced ${data.total_synced} packages (verified in DB: ${data.verified_in_db ?? '?'})`);
      }
      await loadPlans();
    } catch (e) {
  ...
  const deleteAllPlans = async () => {
    ...
    try {
      ...
      toast.success(`Deleted ${data.deleted ?? 'all'} plans`);
      setAllPlans([]);
      await loadPlans();
    } catch (e) {
  ...
  const loadPlans = useCallback(async () => {
    setLoading(true);
    try {
      const url = `/api/admin/plans?_=${Date.now()}`;
      ...
      const data = await res.json();
      console.log('[Plans] API response:', res.status, 'count:', data.plans?.length ?? 0, 'success:', data.success);
      if (!res.ok) throw new Error(data?.error || 'Failed to fetch plans');
      const plans = Array.isArray(data.plans) ? data.plans : [];
      if (plans.length === 0 && data.count > 0) {
        console.warn('[Plans] API says count:', data.count, 'but plans array is empty');
      }
      setAllPlans(plans);
    } catch (e) {
      console.error('[Plans] Error loading plans:', e);
  ...
  // Filter by mode: plans = ALL packages, topups = only topup-eligible
  const modeFiltered = mode === 'topups'
    ? allPlans.filter((p) => p.support_topup === true || p.plan_type === 'topup')
    : allPlans;

  // Filter by tab
  const tabFiltered = modeFiltered.filter((p) => {
  ...
  // Count per tab
  const countryCount = modeFiltered.filter((p) => categorizePlan(p) === 'country').length;
  const globalRegionalCount = modeFiltered.filter((p) => {
    const cat = categorizePlan(p);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-1">
          All eSIM plans from Supabase <code className="text-xs bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">esim_packages</code>. Total: <strong>{allPlans.length}</strong>
        </p>
      </div>

      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
        {/* Plans / Topups Toggle + Category Tabs */}
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="inline-flex rounded-lg bg-gray-100 dark:bg-gray-700 p-1">
              ...
              >
                ...
                Topups ({allPlans.filter(p => p.support_topup === true || p.plan_type === 'topup').length})
              </button>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder={`Search ${mode === 'topups' ? 'topup ' : ''}plans by title, slug, operator...`}
                value={searchQuery}
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {filtered.length} {mode === 'topups' ? 'topup' : 'plan'}{filtered.length !== 1 ? 's' : ''}
              </span>
              <button
                type="button"
                onClick={syncFromAiralo}
                disabled={syncing}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-black text-white hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                <RefreshCw className={`w-4 h-4 ${syncing ? 'animate-spin' : ''}`} />
                {syncing ? 'Syncing...' : mode === 'topups' ? 'Sync All + Topups' : 'Sync from Airalo'}
              </button>
              <button
                type="button"
                onClick={deleteAllPlans}
                disabled={deleting}
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      {mode === 'topups' ? 'No topup plans found' : 'No plans found'}
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                      {searchQuery
                        ? 'Try a different search.'
                        : mode === 'topups'
                          ? 'Click "Sync All + Topups" to import and detect topup-eligible packages.'
                    ...
                    <tr key={plan.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <td className="px-6 py-4">
                        <div>
                          ...
                          {plan.operator && (
                            <div className="text-xs text-gray-500 dark:text-gray-400">{plan.operator}</div>
                          )}
                        </div>
                      </td>
          ...
          <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between sm:px-6">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-2 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
