...
export async function GET(request) {
  try {
    ...
    // Test mode: check URL param OR DB mode
    const testParam = searchParams.get('test');
    const isTest = testParam === '1' || robokassa_mode === 'test';
    const passOne = isTest
      ? (robokassa_test_pass_one || robokassa_pass_one)
      : robokassa_pass_one;
    ...
    // Test mode: add IsTest flag, skip Encoding/SuccessURL/FailURL (cause error 29)
    if (isTest) {
